...
-- 2021-05-22,Laura Truong,Created File
-- 2021-05-25,Laura Truong,Modified script and added code to complete assignment
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_TruongLaura')
	 Begin 
	  Alter Database [Assignment07DB_TruongLaura] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_TruongLaura;
	 End
	Create Database Assignment07DB_TruongLaura;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_TruongLaura;
...
Select ProductName, DateName(Month, InventoryDate) + ', ' + DateName(Year, InventoryDate) As InventoryDate,[Count] As InventoryCount
From dbo.vProducts as p
...
As
Select Top 10000 ProductName, DateName(Month, InventoryDate) + ', ' + DateName(Year, InventoryDate) As InventoryDate, [Count] As InventoryCount
From dbo.vProducts as p
Join dbo.vInventories as i
On p.ProductID = i.ProductID
Order by ProductName, InventoryDate, [Count];
...
As
Select Top 10000 CategoryName, DateName(Month, InventoryDate) + ', ' + DateName(Year, InventoryDate) As InventoryDate, Sum([Count]) As InventoryCountByCategory
...
As
Select Top 1000000000 ProductName, DateName(month, InventoryDate) + ', ' + DateName(year,InventoryDate) As InventoryDate, Sum([Count]) As InventoryCount, IIF(Year(InventoryDate) = 1996, 0, IsNull(Lag(Sum([Count]), 1) Over(Order by ProductName, Month(InventoryDate)), 0)) As PreviousMonthCount
From dbo.vProducts as p
Join dbo.vInventories as i
On p.ProductID = i.ProductID
...
As
Return
Select Top 10000 ProductName, InventoryDate, InventoryCount, PreviousMonthCount, CountVsPreviousCountKPI
